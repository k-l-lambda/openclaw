 ...
 *
 * Re-ported during the 0724-25 upstream rebase against the current plugin-sdk
 * contract: `ChannelOutboundContext` exposes `ctx.to`/`ctx.text` (no
 * `ctx.dedupeKey`); a fresh `crypto.randomUUID()` is used as the messageId.
 * `targetResolver` was removed from `messaging` (it moved to a separate
 * directory/resolver core adapter) and is not needed for this outbound-only
 * channel — `outbound.resolveTarget` (synchronous) handles target validation.
 */
